...
export const CDN_ESM_URL = "https://esm.sh";
export const CDN_SYSTEM_URL = "https://ga.system.jspm.io";
const DEFAULT_DEPENDENCY_VERSIONS = {
  "@vue/component-compiler-utils": "2.1.x",
  less: "2.7.3",
  "source-map": "0.7.3",
  typescript: "2.9.x",
...
export class Runtime extends RegisterLoader {
  ...
  constructor({
    ...
    if (host.getCanonicalPath && typeof host.getCanonicalPath !== "function") {
      throw new TypeError(
          ...
          (hostResolution: string) => {
            if (typeof hostResolution !== "string") {
              return <Promise<string>>(
                Promise.reject(
                  new Error(`Failed to resolve host module '${hostRequest}'`)
                )
              );
            }
        ...
        .then((resolvedPackageJsonKey) => {
          const packageJsonResult =
            typeof resolvedPackageJsonKey === "string"
              ? this.import(resolvedPackageJsonKey).catch((error) =>
                  typeof this.host.getCanonicalPath === "function"
                    ? Promise.reject(
                        new Error(
...
function instantiateVue(
  ...
  return Promise.resolve(transpileResult).then((sourceFileRecord) =>
    instantiateJavascript(runtime, key, sourceFileRecord, processAnonRegister)
  );
}
...
function inlineSourceMap(sourceMap: object | string) {
  ...
  if (typeof btoa !== "undefined")
    return (
      SOURCE_MAP_PREFIX + btoa(unescape(encodeURIComponent(sourceMapString)))
    );
  else return "";
}
...
function loadRemoteModule(
  ...
  if (typeof dynamicImport === "function" && !runtime.useSystem) {
    ...
    return Promise.resolve(dynamicImportResult).then(
      (esModule) => {
      ...
      (err) => {
        console.trace("dynamic import error", key, err.stack);
        throw err;
      }
    );
  }
